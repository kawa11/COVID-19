...
for field in countries:
    ...
    result = {"time": time, "confirmedCount": new_confirmedCount, "confirmedNew": confirmedNew}
    list.append(result)
    # "curedCount": new_curedCount,
    # "deadCount": new_deadCount,
    # "treatCount": treatCount,
    #print(new_confirmedCount)
cursor.close()
conn.close()
with open('json/country.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
def logisticQQ():
    ...
    response = { "mlp_score": mlp_score, "y_train": sum_data, "result": predict_y.tolist(), "x1":np.arange(len(sum_data)+10).tolist()}
    return jsonify(response)
...
@WorldTTrend_api.route('/sjqs/getAllcountry', methods=['GET'])
def getAllcountry():
