...
# 读取数据
def read():
    with open('../csv/DXYArea.csv', encoding='utf-8') as f:
        f_csv = csv.reader(f)
        for row in f_csv:
    ...
    with open('../csv/DXYOverall.csv', encoding='utf-8') as f:
        f_csv = csv.reader(f)
        for row in f_csv:
...
# 写出数据
def write():
    with open('../csv/csv1/chinaArea_data.csv', 'w', newline='', encoding='utf-8') as f:
        f_csv = csv.writer(f)
        for row in china:
            f_csv.writerow(row)
