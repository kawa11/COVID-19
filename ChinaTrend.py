...
for field in result:
    ...
    for i in time:
        for j in time_all:
            if (i == j):
                ...
                new_curedCount.append(curedCount[time_all.index(j)])
                new_deadCount.append(deadCount[time_all.index(j)])
    new_confirmedCount = [new_confirmedCount[i] if i < len(new_curedCount) else new_confirmedCount[-1] for i in
                          range(len(time))]
    ...
    treatCount = [new_confirmedCount[i] - new_curedCount[i] - new_deadCount[i] if (new_confirmedCount[i] -
                                                                                   new_curedCount[i] - new_deadCount[
                                                                                       i]) > 0 else 0
                  for i in range(len(new_confirmedCount))]
    confirmedNew = [
        new_confirmedCount[i + 1] - new_confirmedCount[i] if (new_confirmedCount[i + 1] - new_confirmedCount[
            i]) > 0 else 0
        for i in range(0, len(new_confirmedCount) - 1)]
    curedNew = [new_curedCount[i + 1] - new_curedCount[i] if (new_curedCount[i + 1] - new_curedCount[i]) > 0 else 0 for
                i
...
def hb():
    result = {"time": time, "confirmedCount": HB_confirmedCount, "curedCount": HB_curedCount, "deadCount": HB_deadCount,
              "treatCount": HB_treatCount, "confirmedNew": HB_confirmedNew, "curedNew": HB_curedNew,
              "deadNew": HB_deadNew}
...
def qg():
    result = {"time": time, "confirmedCount": QG_confirmedCount, "curedCount": QG_curedCount, "deadCount": QG_deadCount,
              "treatCount": QG_treatCount, "confirmedNew": QG_confirmedNew, "curedNew": QG_curedNew,
              "deadNew": QG_deadNew}
...
def fhb():
    result = {"time": time, "confirmedCount": FHB_confirmedCount, "curedCount": FHB_curedCount,
              "deadCount": FHB_deadCount,
              "treatCount": FHB_treatCount, "confirmedNew": FHB_confirmedNew, "curedNew": FHB_curedNew,
              "deadNew": FHB_deadNew}
    return jsonify(result)
...
def ssdt():
    confirmedCount = []
    province = [provinces[i][0:3] if ("黑龙江" in provinces[i] or "内蒙古" in provinces[i]) else provinces[i][0:2] for i in
                range(len(provinces))]
    ...
    result = {"provinces": province, "QG_confirmedCount": QG_confirmedCount, "time": time,
...
def nnHB():
    ...
    model_mlp = MLPRegressor(
        hidden_layer_sizes=(100,), activation='logistic', solver='lbfgs', alpha=0.0001, batch_size='auto',
    ...
    response = {"mlp_score": mlp_score, "y_train": y.tolist(), "result": result.tolist(), "x1": x1.tolist(),
                "x": x.tolist()}
